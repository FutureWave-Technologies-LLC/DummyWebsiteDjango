...
# Running the test below gives this error:  django.urls.exceptions.NoReverseMatch: Reverse for 'home' not found. 'home' is not a valid view function or pattern name.
...
# Running the test below gives this error:  django.urls.exceptions.NoReverseMatch: Reverse for 'authenticate_user' not found. 'authenticate_user' is not a valid view function or pattern name.
...
class SignupUserTestCase(TestCase):
    def test_signup_user_valid(self):
        response = self.client.post(reverse('sign_up'), {
            'username': 'newuser', 'password': 'validpass', 'first_name': 'John', 'last_name': 'Doe'
        })
        self.assertEqual(response.status_code, 200)
        self.assertFalse(response.json()["error"])
        print("User saved:", users.objects.filter(username="newuser").exists())
    ...
    def test_signup_user_short_password(self):
        response = self.client.post(reverse('sign_up'), {
            'username': 'newuser', 'password': '123', 'first_name': 'John', 'last_name': 'Doe'
        })
        self.assertEqual(response.status_code, 200)
        self.assertTrue(response.json()["error"])
